...
@Component({
  ...
  templateUrl: './become-a-member.page.html',
  styleUrls: ['./become-a-member.page.scss'],
})

export class BecomeAMemberPage implements OnInit {
      ...
      if(res){
		...
		if(res.userinfo) {
			...
			this.model = {
				...
				username : res.userinfo.system_generated_username,
			...
			// if(res.userinfo.came_to_know_arr){
			// 	res.userinfo.came_to_know_arr.forEach(element => {
			// 		element.came_to_know_arr = res.userinfo.came_to_know_arr;
			// 	});
			// }
  ...
  // ======================== form submit start ===================
	  ...
	  onSubmit(form:NgForm){
	    ...
	    this.formSubmitSubscribe = this.http.post("student/member_signup", fd).subscribe(
	      (response:any) => {
	        ...
	        if(response.return_status > 0){
			  ...
			  if(this.get_user_dtls.approve !== '1'){
